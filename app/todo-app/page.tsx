...
const TodoApp = () => {
  ...
  const handleCheckTodo = (target: number) => {
    ...
    if (itemIndex !== -1) {
      ...
      updatedTodos[itemIndex] = {
        ...updatedTodos[itemIndex],
        state: updatedTodos[itemIndex].state==='active'?'completed':'active',
      }
  ...
  const handleClearCompletedClick = () => {
    setTodos((prevTodos) => prevTodos.filter(todo => todo.state !== 'completed'))
  }
            ...
            {(provided) => (
              <ul className={styles.ul} {...provided.droppableProps} ref={provided.innerRef}>
                ...
                .map((todo, index) => (
                  <Draggable key={todo.id} draggableId={String(todo.id)} index={index}>
                    {(provided) => (
                      ...
                      >
                        <TodoItem id={todo.id} text={todo.text} state={todo.state} checkFunction={handleCheckTodo} removeFunction={handleRemoveTodo} />
                ...
                <li className={ styles.bottomItem }>
                  <p>{(todos.filter(todo => todo.state !== 'completed')).length} item{(todos.filter(todo => todo.state !== 'completed')).length===1?'':'s'} left</p>
                  <div className={ styles.filterDiv }>
                    <button onClick={handleAllClick} className={ filter==='all'?`${styles.actionButton} ${styles.bold} ${styles.active}`:`${styles.actionButton} ${styles.bold}` }>All</button>
                    <button onClick={handleActiveClick} className={ filter==='active'?`${styles.actionButton} ${styles.bold} ${styles.active}`:`${styles.actionButton} ${styles.bold}` }>Active</button>
                    <button onClick={handleCompletedClick} className={ filter==='completed'?`${styles.actionButton} ${styles.bold} ${styles.active}`:`${styles.actionButton} ${styles.bold}` }>Completed</button>
                  </div>
                  <button onClick={handleClearCompletedClick} className={ styles.actionButton }>Clear Completed</button>
                </li>
              </ul>
            )}
          </Droppable>
